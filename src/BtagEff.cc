...
void BtagEff::Loop(string OutputFileName)
{
   ...
   float JetEta[120] = {0};
   int JetFlavor[120] = {0};
   bool JetTagged[120] = {0};
   bool JetTaggedPass1[120] = {0};
   bool JetTaggedPass2[120] = {0};
   bool JetTaggedPass2Up[120] = {0};
   bool JetTaggedPass2Down[120] = {0};
   int RunNumber = 0;
   long long EventNumber = 0;

   //OutputTree.Branch("ModelName", &ModelName, "ModelName/C");
   OutputTree.Branch("JetCount", &JetCount, "JetCount/I");
   OutputTree.Branch("JetPT", JetPT, "JetPT[120]/F");
   OutputTree.Branch("JetEta", JetEta, "JetEta[120]/F");
   OutputTree.Branch("JetFlavor", JetFlavor, "JetFlavor[120]/I");
   OutputTree.Branch("JetTagged", JetTagged, "JetTagged[120]/O");
   OutputTree.Branch("JetTaggedPass1", JetTaggedPass1, "JetTaggedPass1[120]/O");
   OutputTree.Branch("JetTaggedPass2", JetTaggedPass2, "JetTaggedPass2[120]/O");
   OutputTree.Branch("JetTaggedPass2Up", JetTaggedPass2Up, "JetTaggedPass2Up[120]/O");
   OutputTree.Branch("JetTaggedPass2Down", JetTaggedPass2Down, "JetTaggedPass2Down[120]/O");
   ...
   for(long iEntry = 0; iEntry < nentries; iEntry++)
   {
      ...
      */
      string Model = "monoB/monoB_300GeV_Tight";
      strcpy(ModelName, Model.c_str());
      RunNumber = runNumber;
      ...
      bool OriginalTag[15000] = {false};
      bool Pass1Tag[15000] = {false};
      bool Pass2TagCentral[15000] = {false};
      bool Pass2TagUp[15000] = {false};
      bool Pass2TagDown[15000] = {false};
      for(int i = 0; i < nAK5PFNoPUJet; i++)
      {
         double eta = etaAK5PFNoPUJet[i];
         double phi = phiAK5PFNoPUJet[i];
         //double csv = combinedSecondaryVertexBJetTagsAK5PFPUcorrJet[i];//Use the collection YOUR ANALYSIS USES!
	 double csv = combinedSecondaryVertexBJetTagsAK5PFNoPUJet[i];
	 ...
	 if(eta < 2.4 && eta > -2.4 && pt > 40){
	   //OriginalTag[i] = (csv > 0.244);//Loose
	   //OriginalTag[i] = (csv > 0.679);//Medium
	   OriginalTag[i] = (csv > 0.898);//Tight
...
void BtagEff::ReadFile(vector<double> &PT, vector<double> &Eta,
   double Array[15000][5], double Error[15000][5], string FileName, bool DoubleError)
{
...
void BtagEff::InitializeAllArrays()
{
   ...
   double DummyError[15000][5];
   
   ReadFile(B_SF_PTBins, B_SF_EtaBins, B_SF, B_SF_Error,
	    "BTagging/8TeVNew/CP/SF_files/Beff_SF_CSVT_cpena.txt", false);
   ReadFile(C_SF_PTBins, C_SF_EtaBins, C_SF, C_SF_Error,
	    "BTagging/8TeVNew/CP/SF_files/Ceff_SF_CSVT_cpena.txt", false);
   ReadFile(L_SF_PTBins, L_SF_EtaBins, L_SF, L_SF_Error,
	    "BTagging/8TeVNew/CP/SF_files/Leff_SF_CSVT_cpena.txt", false);
   
   
   ReadFile(B_SFFast_PTBins, B_SFFast_EtaBins, B_SFFast, B_SFFast_Error,
	    "BTagging/8TeVNew/CP/SF_files/BEff_SF_FastSim_Dummy.txt", false);
   ReadFile(C_SFFast_PTBins, C_SFFast_EtaBins, C_SFFast, C_SFFast_Error,
	    "BTagging/8TeVNew/CP/SF_files/CEff_SF_FastSim_Dummy.txt", false);
   ReadFile(L_SFFast_PTBins, L_SFFast_EtaBins, L_SFFast, L_SFFast_Error,
	    "BTagging/8TeVNew/CP/SF_files/Mistag_SF_FastSim_Dummy.txt", false);
